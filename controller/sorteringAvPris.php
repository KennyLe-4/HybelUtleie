<?php
// Denne klassen sortere prisen på annonsene. Funksjonen hentPrisStigen, henter billigste annonsene førsst. Funksjonen hentPrisSynkende, henter de dyreste annonsene først. 
class Pris
{
    public function hentPrisStigende()
    {

        global $pdo;
        $q = $pdo->prepare("SELECT * FROM `annonser` ORDER BY `annonser`.`pris` DESC");
...
public function hentPrisSynkende() 
{
   
    global $pdo;
    $q = $pdo->prepare("SELECT * FROM `annonser` ORDER BY `annonser`.`pris` ASC");
    //forbereder queryen opp mot DB
